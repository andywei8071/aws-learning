...
df = pd.DataFrame({'type': r_type, 'vcpu': r_vcpu, 'memory': r_memory, 'location': r_location, "tenancy": r_tenancy, "os": r_operatingSystem, "all_upfront_price_1yr": reserve_price['1yr_all'], "partial_upfront_price_1yr": reserve_price['1yr_partial'], "no_upfront_price_1yr": reserve_price['1yr_no'], "on_demand_price": on_demand_price})

df[['vcpu', 'memory', 'all_upfront_price_1yr', 'partial_upfront_price_1yr', 'no_upfront_price_1yr', 'on_demand_price']] = df[['vcpu', 'memory', 'all_upfront_price_1yr', 'partial_upfront_price_1yr', 'no_upfront_price_1yr', 'on_demand_price']].astype('float')

df.drop_duplicates(subset=['type', 'vcpu', 'memory', 'location', 'os', 'all_upfront_price_1yr', 'partial_upfront_price_1yr', 'no_upfront_price_1yr', 'on_demand_price'], keep='first', inplace=True)
#df.drop_duplicates(subset=['type', 'vcpu', 'memory', 'location', 'tenancy', 'os', 'all_upfront_price_1yr', 'partial_upfront_price_1yr', 'no_upfront_price_1yr', 'all_upfront_price_3yr','partial_upfront_price_3yr', 'no_upfront_price_3yr', 'on_demand_price', 'capacity_status', 'preInstalledSw', 'license_model'], keep='first', inplace=True)
df = df[(df.all_upfront_price_1yr > 0) | (df.partial_upfront_price_1yr > 0) | (df.no_upfront_price_1yr > 0) | (df.on_demand_price > 0)]
